...
namespace Assets.Scripts.IA
{
    public class Master
    {
        public static void Main(string[] args)
        {
            ...
            status[2]= new Status("Maldição do medo",5, 90,Stat.def, -1);
            status[4]=  new Status("Confundir",5, 90,Stat.spatk, -1);
            status[5]=  new Status("Perda de fé",5, 90,Stat.spdef, -1);
            status[6]=  new Status("Benção de força",5, 90,Stat.atk, 1);
            status[7]=  new Status("Pele de pedra",5, 90,Stat.def, 1);
            status[8]=  new Status("Benção de Atena",5, 90,Stat.spatk, 1);
            status[9]=  new Status("Benção de Hermes",5, 90,Stat.velocidade, 1);
            status[10]=  new Status("Armadura mágica",5, 90,Stat.spdef, 1);
